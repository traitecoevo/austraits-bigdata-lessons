...
data <- austraits$traits #get data

data %>%
  #create new data set with listed variables
  select(taxon_name, trait_name) %>%
  # splits by both taxon name and then trait name
  data_count <-
    data %>%
...
data_full <- 
  expand_grid(
    taxon_name = data$taxon_name %>% unique() %>% sample(2000),
    trait_name = data$trait_name %>% unique()
  )

data_coverage <- 
  data_full %>% 
  left_join(data_count) %>% 
  mutate(trait_presence = ifelse(is.na(n), 0, 1))

# Make the plot
data_coverage %>% 
  mutate(
    #reorder categorical variables by rank in coverage
    trait_name = forcats::fct_reorder(trait_name, trait_presence, sum, .desc = T), 
    taxon_name = forcats::fct_reorder(taxon_name, trait_presence, sum, .desc = T)
  ) %>%
  ggplot(aes(trait_name, taxon_name, fill = as.character(trait_presence))) +
