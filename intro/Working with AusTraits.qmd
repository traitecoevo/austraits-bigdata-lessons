...
A list is simply that, an ordered list of any combination of items, such as integers, character strings, vectors, or even a list of lists! 
...
Most parts are tables (a.k.a. tibbles)
...
2. From the taxa table, how many accepted species are there?
...
4. What is the range of values in these 3 traits: flower_colour, seed_N_per_seed_dry_mass, leaf_dry_mass

```{r}
austraits_subset <- austraits$traits %>% select(trait_name, value, unit, taxon_name)

austraits_subset %>% filter(trait_name == "flower_colour") %>% unique(value)

austraits_subset %>% filter(trait_name == "seed_N_per_seed_dry_mass") %>% range(value)

austraits_subset %>% filter(trait_name == "leaf_dry_mass") %>% range(value)
```

Answer: 


5. what proportion of traits have integer (not character) values?
...
7. Let's have a look at the wood_density trait, made a new data frame object and isolate this trait and the taxon_name, trait_name, value and unit columns
...
wood_density <- wood_density %>% 
  mutate(wood_density_kg = as.integer(value)*1E06)
...
9. How many contributors from Australian institutions? (too hard?) Top contributor?
...
1. Plot values for wood density above
```{r}
wood_density %>%
  ggplot(aes(as.integer(value))) +
  geom_histogram(bins = 10, binwidth = ) +
  theme_classic()
```
