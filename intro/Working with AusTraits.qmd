...
```{r}
austraits$taxa$taxonomic_status %>% 
  filter(taxonomic_status == "") %>% length()
```
...
```{r}
austraits %>% group_by(traits$trait_name) %>%
  sort(traits$value, decreasing = TRUE)

austraits %>% group_by(traits$trait_name) %>% nrow(traits$observation_id)
...
```{r}
austraits <- austraits$traits %>% tibble(trait_name, value, unit, taxon_name) 
